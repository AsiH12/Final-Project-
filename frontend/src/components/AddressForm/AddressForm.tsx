...
export function AddressForm({ onSaveAddress }: AddressFormProps) {
  ...
  useEffect(() => {
    ...
    fetchAddresses().catch((error) => console.error("Error fetching addresses:", error));
  }, []);
  ...
  const handleSaveAddress = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5000/addresses${isEditing ? `/${currentAddress?.id}` : ''}`, {
      ...
      if (isEditing) {
        setAddresses((prevAddresses) =>
          prevAddresses.map((address) => (address.id === currentAddress?.id ? updatedAddress : address))
        );
      } else {
