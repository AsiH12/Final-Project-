...
def get_all_discounts_shops():
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT id, shop_id, discount, expiration_date, minimum_amount, allow_others FROM discounts_shops")
...
def get_discount_shop_by_id(discount_id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT id, shop_id, discount, expiration_date, minimum_amount, allow_others FROM discounts_shops WHERE id = ?", (discount_id,))
...
def create_discount_shop():
    ...
    if request.is_json:
        ...
        if not all([shop_id, discount, expiration_date, minimum_amount, allow_others]):
            return jsonify({"error": "Incomplete discount data"}), 400

        db = get_db()
        cursor = db.cursor()
        cursor.execute("INSERT INTO discounts_shops (shop_id, discount, expiration_date, minimum_amount, allow_others) VALUES (?, ?, ?, ?, ?)", (shop_id, discount, expiration_date, minimum_amount, allow_others))
        db.commit()
...
def update_discount_shop(discount_id):
    ...
    if request.is_json:
        ...
        cursor.execute("UPDATE discounts_shops SET shop_id = ?, discount = ?, expiration_date = ?, minimum_amount = ?, allow_others = ? WHERE id = ?", (shop_id, discount_value, expiration_date, minimum_amount, allow_others, discount_id))
        db.commit()
